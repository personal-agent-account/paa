...
// PBI-0154 AC-3/X1/X2: bun も cargo も無い配布先で `atn login` が broker binary を
// 公開 Release から取ってくる。取得の形自体(checksum 必須・tmp→rename)は PBI-0137 の
// ensureBinary と共通なので、ここでは name="atn-broker" の 3 経路(取得成功・checksum 不一致・
// 未対応 OS/arch)だけを見る(binary-fetch.test.ts の atn-mcp 版と対称)。
//
// `atn login` からの呼び出し配線(ensureBrokerBinary → resolveBrokerBin の download 先追加)は
// 実 repo checkout に broker/target/{release,debug}/atn-broker が既に存在しうる dev tree の上で
...
const binPath = () => join(home, "bin", "atn-broker");

beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), "atn-broker-binfetch-"));
  requests = [];
  mode = "ok";
});
...
describe('ensureBinary("atn-broker") — 取得と検証 (PBI-0154)', () => {
  test("AC-3: bin が空・network 有りなら公開 Release から download して実行可能な状態で置く", async () => {
    const outcome = await ensureBinary("atn-broker", { env: envFor() });
  ...
  test("AC-3: 同じ version が既に置いてあれば再 download しない", async () => {
    await ensureBinary("atn-broker", { env: envFor() });
  ...
  // broker だけ旧版が residual に残る。stamp が古ければ引き直すことを name="atn-broker" でも見る
...
describe("release workflow の broker 面 (PBI-0154 AC-2)", () => {
  test("darwin-arm64 / darwin-x64 / linux-x64 の atn-broker asset を組み立てて SHA256SUMS に含める", async () => {
    const yml = await readFile(
      fileURLToPath(new URL("../../../.github/workflows/release.yml", import.meta.url)),
      "utf8",
    );
    for (const target of ["darwin-arm64", "darwin-x64", "linux-x64"]) {
      expect(yml).toContain(`dist/atn-broker-${target}`);
    }
    expect(yml).toContain("atn-broker-*"); // SHA256SUMS の対象に含まれる
...
describe("launcher(sh)と ensureBinary(TS)の既定 version (PBI-0154 review)", () => {
  test("同じ version を既定にする — 片方だけ上げると plugin 経路が存在しない tag を引きに行く", async () => {
    ...
    const sh = await readFile(fileURLToPath(new URL("packages/mcp/atn-mcp", root)), "utf8");
    const claude = await readFile(
      fileURLToPath(new URL("adapters/official/claude/atn-mcp", root)),
      "utf8",
    );
    ...
    const codex = await readFile(
      fileURLToPath(new URL("adapters/official/codex/atn-mcp", root)),
      "utf8",
    );
