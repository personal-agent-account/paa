//! `atn` CLI をどう起こすかの 1 箇所(PBI-0070 で adopt.rs から切り出し)。
//!
//! broker は 2 つの用途で TS 側の CLI を起こす: 自動登録の materialize(`atn adopt`。PBI-0023)と、
//! 外部 API provider を端末側 runtime として動かす wake(`atn agent`。EP-0009 C)。どちらも
//! 「dev の repo checkout では `atn` が PATH に無い」という同じ事情を抱えるので、argv0 の
//! 解決を 2 箇所に写さない。

/// 起こす CLI の argv。`PAA_CLI`(既定 `atn`)。`:` 区切りで argv0 + 先行引数も書ける ——
...
pub fn cli_argv_from(raw: Option<String>) -> Vec<String> {
    raw.unwrap_or_else(|| "atn".to_string())
        .split(':')
        .filter(|s| !s.is_empty())
        .map(str::to_string)
        .collect()
...
mod tests {
    ...
    #[test]
    fn defaults_to_paa() {
        assert_eq!(cli_argv_from(None), vec!["atn".to_string()]);
    }
